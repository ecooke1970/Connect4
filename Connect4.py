...
def draw_board(self):
    ...
    my_btn1 = tk.Button(top_frame, text="C1", command=player_move(self, 1))
    ...
    # my_canvas.create_oval(500, 500, 550, 550, fill="red")
...
class Connect4:
    ...
    def play(self):
        ...
        while not self.winner:
            ...
            top_frame = tk.Frame(window)
            my_btn1 = Button(top_frame, text="C1", command=Connect4.player_move(self, 1))
            my_btn2 = Button(top_frame, text="C2")
            my_btn3 = Button(top_frame, text="C3")
            my_btn4 = Button(top_frame, text="C4")
            my_btn5 = Button(top_frame, text="C5")
            my_btn6 = Button(top_frame, text="C6")
            my_btn7 = Button(top_frame, text="C7")
            top_frame.pack()
            my_btn1.grid(row=0, column=0)
            my_btn2.grid(row=0, column=1)
            my_btn3.grid(row=0, column=2)
    ...
    def player_move(self, column):
        ...
        if valid_move:
            ...
            if self.moves > 6:
                self.winner = Connect4.check_for_winner(self)
    ...
    def check_for_winner(self):
        ...
        for i in range(self.board.get_num_columns()):
            if (self.player * 4) in self.board.get_diagonal(row=0, column=i, direction="dr"):
                return True
            if (self.player * 4) in self.board.get_diagonal(row=self.board.get_num_rows() - 1, column=i, direction="ur"):
                return True
